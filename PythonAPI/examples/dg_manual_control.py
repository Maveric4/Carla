...
class World(object):
    ...
    def dg_spawn_npc(self, client, nbr_vehicles):
        ...
        for response in client.apply_batch_sync(batch):
            if response.error:
                ...
                self._actor_list.append(response.actor_id)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if self._is_quit_shortcut(event.key):
                    ...
                    world.restart()
                ## #1 Spawning 10 npc when pressing F10 on keyboard
                elif event.key == K_F10:
                    world.dg_spawn_npc(client, nbr_vehicles=10)
                ## End of #1
                ## #2 Making shot if F9 pressed
                elif event.key == K_F9:
...
class CameraManager(object):
    ...
    def _parse_image(weak_self, image):
        ...
        if self.sensors[self.index][0].startswith('sensor.lidar'):
            ...
            array = array[:, :, :3]
            if self.sensors[self.index][0].startswith('sensor.camera.semantic_segmentation'):
                if self.do_shot:
                    cv2.imwrite('_out/sem_img_{}.jpg'.format(image.frame_number), array)
